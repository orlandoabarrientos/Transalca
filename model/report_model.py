...
class ReportModel(Connection):
    ...
    def get_recent_orders(self, limit=10):
        ...
        for order in orders:
            client = self.fetch_one("mantenimiento", "SELECT nombre, apellido FROM usuarios WHERE id = %s", (order['cliente_id'],))
    ...
    def get_sales_history(self, start_date=None, end_date=None, status=None):
        sql = "SELECT id, cliente_id, fecha, total, estado FROM ordenes_venta WHERE 1=1"
        params = []
        if start_date:
    ...
    def get_payments_history(self, start_date=None, end_date=None, status=None):
        sql = "SELECT c.id, o.cliente_id, c.orden_venta_id as orden_id, CONCAT('IMG-', c.id) as referencia, o.total as monto, 'USD' as moneda, o.metodo_pago as metodo, c.fecha, c.estado FROM comprobantes_pago c INNER JOIN ordenes_venta o ON c.orden_venta_id = o.id WHERE 1=1"
        params = []
        if start_date:
        ...
        for p in payments:
            client = self.fetch_one("mantenimiento", "SELECT nombre, apellido FROM usuarios WHERE id = %s", (p['cliente_id'],))
    ...
    def get_inventory_kardex(self, start_date=None, end_date=None):
        ...
        sql = "SELECT i.id, i.producto_id, 'Inventario Actual' as tipo, i.stock as cantidad, 'Ajuste Sistémico' as motivo, i.updated_at as fecha, p.nombre as producto, p.codigo FROM inventario i INNER JOIN productos p ON i.producto_id = p.id WHERE 1=1"
        params = []
        if start_date:
    ...
    def get_mechanics_performance(self, start_date=None, end_date=None):
        ...
        for r in records:
            ...
            if mid not in mecanicos_stats:
                mec = self.fetch_one("transalca", "SELECT nombre, apellido FROM mecanicos WHERE id = %s", (mid,))
                if mec:
