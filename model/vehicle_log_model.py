...
class VehicleLogModel(Connection):
    ...
    def _vehicle_plate(self, vid):
        vehicle = self.fetch_one("transalca", "SELECT placa FROM vehiculos WHERE id=%s", (vid,))
    ...
    def get_by_vehicle(self, vid, limit=50):
        return self.fetch_all("transalca",
            ...
            "WHERE bv.vehiculo_placa=(SELECT placa FROM vehiculos WHERE id=%s) "
            "ORDER BY bv.fecha DESC LIMIT %s", (vid, limit))
    ...
    def create(self, data):
        ...
        return self.insert("transalca",
            "INSERT INTO bitacora_vehiculo (vehiculo_placa, orden_venta_id, "
            ...
            (placa, data.get('orden_venta_id'), data.get('servicio_id'),
    ...
    def count_oil_changes(self, vid):
        result = self.fetch_one("transalca",
            ...
            "WHERE vehiculo_placa=(SELECT placa FROM vehiculos WHERE id=%s) "
            "AND aceite_usado IS NOT NULL AND aceite_usado != ''", (vid,))
        return result['total'] if result else 0

    def get_service_count(self, vid, tipo_servicio=None):
        sql = "SELECT COUNT(*) as total FROM bitacora_vehiculo WHERE vehiculo_placa=(SELECT placa FROM vehiculos WHERE id=%s)"
        params = [vid]
        if tipo_servicio:
