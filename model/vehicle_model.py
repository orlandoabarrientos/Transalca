...
class VehicleModel(Connection):
    ...
    def get_by_cliente(self, cliente_cedula):
        return self.fetch_all("transalca",
            "SELECT * FROM vehiculos WHERE cliente_cedula = %s AND estado = 1 "
    ...
    def update_vehicle(self, vid, data):
        ...
        return self.update("transalca",
            ...
            (data['marca'].strip(), data['modelo'].strip(),
             ...
             (data.get('filtros_info') or '').strip(),
             (data.get('refrigerante_info') or '').strip(),
             (data.get('observaciones') or '').strip(), vid))
    ...
    def update_carnet_image(self, vid, filename):
        return self.update("transalca",
            "UPDATE vehiculos SET imagen_carnet = %s WHERE id = %s", (filename, vid))

    def soft_delete(self, vid):
        return self.update("transalca",
            "UPDATE vehiculos SET estado = 0 WHERE id = %s", (vid,))
    ...
    def placa_exists(self, placa, exclude_id=None):
        ...
        if exclude_id:
            return self.fetch_one("transalca",
                "SELECT id FROM vehiculos WHERE placa = %s AND id != %s",
                (placa.upper(), exclude_id)) is not None
        return self.fetch_one("transalca",
            "SELECT id FROM vehiculos WHERE placa = %s", (placa.upper(),)) is not None
    ...
    def get_cauchos(self, vid):
        v = self.fetch_one("transalca",
            "SELECT cauchos_json FROM vehiculos WHERE id = %s", (vid,))
    ...
    def save_cauchos(self, vid, cauchos_data):
        return self.update("transalca",
            "UPDATE vehiculos SET cauchos_json = %s WHERE id = %s",
            (json.dumps(cauchos_data, ensure_ascii=False), vid))
    ...
    def update_caucho(self, cid, data):
        vehicle = self.fetch_one("transalca",
            "SELECT id, cauchos_json FROM vehiculos WHERE JSON_CONTAINS(cauchos_json, JSON_OBJECT('id', %s))",
            (cid,))
        if not vehicle:
            return 0
        items = self.get_cauchos(vehicle['id'])
        for item in items:
            if int(item.get('id', 0)) == int(cid):
                ...
                return self.save_cauchos(vehicle['id'], items)
        return 0
    ...
    def delete_caucho(self, cid):
        ...
        if not vehicle:
            return 0
        items = [i for i in self.get_cauchos(vehicle['id']) if int(i.get('id', 0)) != int(cid)]
        return self.save_cauchos(vehicle['id'], items)
