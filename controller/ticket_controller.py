...
def get_all():
    try:
        ...
        return jsonify({"status": "success", "data": model.ejecutar("get_all", estado, prioridad)})
...
def get_one(tid):
    try:
        ...
        if auth:
            return auth
        t = model.ejecutar("get_by_id", tid)
        if not t:
...
def update_status(tid):
    try:
        ...
        validate_choice(estado, ESTADOS_TICKET, 'estado')
        model.ejecutar("update_status", tid, estado)
        ticket = model.ejecutar("get_by_id", tid)
        if ticket:
            user = model.ejecutar("user_id_by_cedula", ticket['cliente_cedula'])
            if user:
                notif_model.ejecutar("notify_ticket_update", user['id'], tid,
...
def reply(tid):
    try:
        ...
        rid = model.ejecutar("add_response", {
            'ticket_id': tid,
...
def stats():
    try:
        ...
        if not is_employee():
            return deny()
        return jsonify({"status": "success", "data": model.ejecutar("count_by_estado")})
    except Exception as e:
        return jsonify({"status": "error", "message": "No se pudo completar la solicitud."}), 500
