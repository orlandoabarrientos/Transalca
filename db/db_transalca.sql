...
AFTER UPDATE ON tarjeta_fidelidad
FOR EACH ROW
BEGIN
    DECLARE v_puntos_req INT;
    IF NEW.canjeada = 0 THEN
        SELECT puntos_requeridos INTO v_puntos_req FROM promociones WHERE id = NEW.promocion_id;
        IF NEW.puntos_acumulados >= v_puntos_req THEN
            UPDATE tarjeta_fidelidad SET canjeada = 1 WHERE id = NEW.id;
        END IF;
    END IF;
END //
DELIMITER ;
